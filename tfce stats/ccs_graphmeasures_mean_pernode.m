...
% Added code to get GT measures per node
...
graphdata{2,1} = 'global efficiency';
graphdata{3,1} = 'centrality';
graphdata{4,1} = 'participation coefficient';
graphdata{5,1} = 'degree';
...
tvals = 1:-0.025:0.1;
% The below portion takes 76 minutes to compute
           ...
           if sum(isnan(check_valid_conn(:))) == 0
               ...
               for band = 1:size(setsize_conn_mean_GT,2)
                   ...
                   for thresh = 1:length(tvals)
                       ...
                       alleff(thresh,:) = efficiency_bin(bin_connmat,1); % 1 computes local eff
                       ...
                       graphdata{1,2}(setsize,band,subject_no,thresh,:) = allcc(thresh,:);
                       
                       % global efficiency
                       graphdata{2,2}(setsize,band,subject_no,thresh,:) = alleff(thresh,:);

                       % betweenness centrality
                       graphdata{3,2}(setsize,band,subject_no,thresh,:) = allbet(thresh,:);
                        
                       % participation coefficient
                       graphdata{4,2}(setsize,band,subject_no,thresh,:) = mean(squeeze(allpc(thresh,:,:)));
                        
                       % degree
                       graphdata{5,2}(setsize,band,subject_no,thresh,:) = alldeg(thresh,:);
                   end
               end
           end
       end
...
save('results/pernode_GT.mat','graphdata');